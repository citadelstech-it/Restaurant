...
const Checkout = () => {
  ...
  const calculateTotal = () =>
    items.reduce(
      (sum, item) => sum + (item.Item?.price || item.price || 0) * (item.quantity || 1),
      0
    );
  ...
  const generateTransactionId = () => {
    const now = new Date();
    return `tnx${now.getFullYear()}${String(now.getMonth() + 1).padStart(2, "0")}${String(now.getDate()).padStart(2, "0")}${String(now.getHours()).padStart(2, "0")}${String(now.getMinutes()).padStart(2, "0")}${String(now.getSeconds()).padStart(2, "0")}`;
  };

  const handleSubmit = async (e) => {
    e.preventDefault();

    const { customer_name, customer_phone, customer_email, paymentMethod } = formData;
            ...
            {items.length === 0 ? (
              ...
              items.map((item) => (
                <p key={item.id || item._id}>
                  ...
                  <strong>₹{(item.Item?.price || item.price || 0).toFixed(2)}</strong>
                </p>
              ))
            )}
            <hr />
